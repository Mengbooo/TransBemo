...
> **推荐**：由于底层识别使用的是 PCM 格式，因此推荐直接上传 PCM 格式。若上传其它格式，在服务器端会有额外转换为 PCM 格式的工作，从而增加一定的调用耗时。
...
- **API HTTPS地址**：`https://fanyi-api.baidu.com/api/trans/v2/voicetrans`
- **请求方式**：POST
...
| X-Appid | `appid` | 是 | 从管理控制台中获得 |
| X-Timestamp | `1642331332` | 是 | Unix时间戳（10位） |
| X-Sign | `i1b/6Ml/NxhEhOnsiQIK7nqsPH/avhhlHgfpo84lJC8=` | 是 | 请求签名 |

##### Body 参数

| 参数名 | 参数类型 | 是否必填 | 示例 | 备注 |
|--------|----------|----------|------|------|
| from | string | 是 | `en` | 源语言的语言代码 |
| to | string | 是 | `zh` | 目标语言的语言代码 |
| voice | string | 是 | `W3NvdXJjZSBhdWRpbyBieXRlc10K` | 语音文件二进制数据，base64编码 |
| format | string | 是 | `pcm` | 音频文件格式 |
...
| 10004 | 访问频率受限 | 请降低调用频率 |
| 10005 | 签名错误 | 请检查签名生成方法 |
| 10006 | 未授权用户 | 检查 APP ID 是否正确或服务是否开通 |
| 10007 | 客户端IP非法 | 检查填写的IP地址是否正确 |
| 20000 | 语言方向不支持 | 检查源语种或目标语种 |
| 20200 | 语音翻译请求失败 | 请重试 |
| 20201 | 音频数据大小超过限制 | 上传符合大小要求的音频数据 |
| 20202 | 文件格式不支持 | 上传符合格式要求的音频数据 |
...
| 错误码 | 含义 |
|--------|------|
| 3001-3010 | 录音机和文件操作异常 |
| 3100-3102 | VAD（语音活动检测）异常 |
